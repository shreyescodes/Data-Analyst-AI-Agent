...
class Visualization:
    ...
    def create_chart(
        self, data: pd.DataFrame, chart_config: Dict[str, Any]
    ) -> go.Figure:
    ...
    def create_histogram(
        self, data: pd.DataFrame, x_col: str, title: str = "Histogram"
    ) -> go.Figure:
        """Create a histogram"""
    ...
    def create_heatmap(
        self, data: pd.DataFrame, title: str = "Correlation Heatmap"
    ) -> go.Figure:
    ...
    def create_distribution_plot(self, data: pd.DataFrame, column: str) -> go.Figure:
        ...
        # Histogram
        fig.add_trace(
            go.Histogram(x=data[column], name="Distribution", nbinsx=30), row=1, col=1
        )

        # Box plot
        fig.add_trace(
            go.Box(x=data[column], name="Box Plot", orientation="h"), row=2, col=1
        )
    ...
    def create_summary_dashboard(self, data: pd.DataFrame) -> go.Figure:
        ...
        if len(numeric_cols) > 0:
            fig.add_trace(
                go.Histogram(x=data[numeric_cols[0]], showlegend=False), row=row, col=1
            )
            row += 1
