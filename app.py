...
def main():
    ...
    st.markdown(
        "Upload your dataset, clean it automatically, and get AI-powered insights!"
    )
...
def handle_data_ingestion():
    ...
    if ingestion_method == "Upload File":
        ...
        uploaded_file = st.file_uploader(
            "Choose a file", type=["csv", "xlsx"], help="Maximum file size: 100MB"
        )
            ...
            if validate_file_size(uploaded_file, max_size_mb=100):
                with st.spinner("Loading data..."):
                    try:
                        ...
                        st.success(
                            f"✅ Data loaded successfully! Shape: {st.session_state.data.shape}"
                        )

                        # Show preview
                        st.subheader("Data Preview")
                        st.dataframe(
                            st.session_state.data.head(), use_container_width=True
                        )
                        ...
                        with col3:
                            st.metric(
                                "Memory Usage",
                                format_bytes(
                                    st.session_state.data.memory_usage(deep=True).sum()
                                ),
                            )
        ...
        if st.button("Connect to API"):
            if api_url:
                with st.spinner("Connecting to API..."):
                    try:
                        st.session_state.data = data_ingestion.load_from_api(
                            api_url, api_key, headers_input
                        )
                        st.session_state.current_step = max(
                            st.session_state.current_step, 2
                        )
...
def handle_data_cleaning():
    ...
    if st.button("🧹 Clean Data"):
        with st.spinner("Cleaning data..."):
            try:
                ...
                cleaned_data, cleaning_report = data_cleaner.clean_data(
                    st.session_state.data, cleaning_options
                )
    ...
    commit_message = st.text_input(
        "Commit message for this version:", "Auto-cleaning performed"
    )

    if st.button("💾 Save Version Snapshot"):
        vid = st.session_state.version_control.save_version(
            st.session_state.cleaned_data, message=commit_message
        )
        ...
        if versions:
            for v in versions:
                with st.expander(f"Version {v['version_id']}"):
                    ...
                    if st.button(
                        f"🔄 Load Version {v['version_id']}", key=v["version_id"]
                    ):
                        st.session_state.cleaned_data = (
                            st.session_state.version_control.load_version(
                                v["version_id"]
                            )
                        )
...
def handle_storage_and_queries():
    ...
    if st.button("💾 Store Data in Database"):
        with st.spinner("Storing data..."):
            try:
                table_name = st.session_state.db_manager.store_data(
                    st.session_state.cleaned_data
                )
    ...
    if not st.session_state.ai_services.use_ai:
        st.info(
            "ℹ️ Using rule-based SQL generation. Add HF_API_KEY in Settings for AI-powered queries."
        )
    ...
    if user_question and st.button("🔍 Analyze"):
        ...
        if ai_services.last_error:
            st.warning(
                f"⚠️ {ai_services.last_error} - Using rule-based fallback. Check Settings to validate your HF token."
            )
        ...
        with st.spinner("Converting question to SQL and analyzing..."):
            try:
                ...
                if not ai_services.use_ai:
                    st.caption(
                        "⚠️ Generated using keyword matching (no AI). Configure HF_API_KEY for better results."
                    )
                ...
                if not ai_services.use_ai:
                    st.caption(
                        "⚠️ Basic statistical insights. Configure HF_API_KEY for AI-powered analysis."
                    )

                # Show error if it was set during execution
                if ai_services.last_error:
                    st.warning(
                        f"⚠️ {ai_services.last_error} - Results generated using rule-based fallback."
                    )
...
def handle_visualization():
    ...
    if not ai_services.use_ai:
        st.info(
            "ℹ️ Using rule-based visualization suggestions (works great without AI!)"
        )
        ...
        with st.spinner("Creating visualization..."):
            try:
                ...
                fig = visualizer.create_chart(
                    st.session_state.cleaned_data, chart_config
                )
                st.plotly_chart(fig, use_container_width=True)

                # Generate explanation
                explanation = ai_services.explain_visualization(
                    viz_question, chart_config
                )
    ...
    numeric_cols = st.session_state.cleaned_data.select_dtypes(
        include=["number"]
    ).columns.tolist()
    categorical_cols = st.session_state.cleaned_data.select_dtypes(
...
def handle_profiling():
    ...
    if st.button("📊 Generate Profiling Report"):
        with st.spinner("Generating comprehensive data profile..."):
            try:
                report_path = profiler.generate_profile_report(
                    st.session_state.cleaned_data
                )
