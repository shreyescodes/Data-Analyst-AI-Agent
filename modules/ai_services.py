...
class AIServices:
    ...
    def validate_token(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_payload = {"inputs": "Hello", "parameters": {"max_new_tokens": 10}}

            response = requests.post(
                test_url, headers=self.headers, json=test_payload, timeout=10
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 401 or response.status_code == 403:
                    ...
                    raise Exception(
                        "Hugging Face API token is missing or invalid. Using rule-based fallback."
                    )
                ...
                if response.status_code == 503:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(
                            "Model is currently loading. Using rule-based fallback."
                        )
                ...
                if isinstance(result, list) and len(result) > 0:
                    ...
                    raise Exception(
                        f"Unexpected response format. Using rule-based fallback."
                    )
                ...
                if attempt < max_retries - 1:
                    ...
                    continue
                raise Exception(
                    f"API request failed: {str(e)}. Using rule-based fallback."
                )

        raise Exception(
            "Failed to get response from model after retries. Using rule-based fallback."
        )

    def natural_language_to_sql(
        self, question: str, columns: List[str], dtypes: Dict[str, Any]
    ) -> str:
...
SQL Query:"""
        ...
        try:
            sql_query = self._query_hf_model(
                self.sql_model, prompt, max_tokens=256, temperature=0.1
            )
    ...
    def _generate_fallback_sql(
        ...
        if "group by" in question_lower or "by" in question_lower:
            categorical_cols = [
                col for col, dtype in dtypes.items() if "object" in str(dtype)
            ]
            numeric_cols = [
                col
                for col, dtype in dtypes.items()
    ...
    def suggest_visualization(
        self, question: str, columns: List[str], sample_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if any(
            ...
            for word in ["trend", "over time", "time series", "change", "evolution"]
        ):
        ...
        if any(
            word in question_lower
            for word in ["distribution", "spread", "histogram", "frequency"]
        ):
            if numeric_cols:
                return {
        ...
        if any(
            word in question_lower
            for word in ["compare", "comparison", "versus", "vs", "by category", "by"]
        ):
            if categorical_cols and numeric_cols:
                return {
                    "chart_type": "bar",
        ...
        if any(
            word in question_lower
            for word in ["correlation", "relationship", "scatter", "relation"]
        ):
        ...
        if any(
            word in question_lower
            for word in ["proportion", "composition", "pie", "percentage", "share"]
        ):
            if categorical_cols:
                return {
                    "chart_type": "pie",
...
Analysis:"""
        ...
        try:
            insights = self._query_hf_model(
                self.text_model, prompt, max_tokens=512, temperature=0.5
            )
            return insights
        ...
        if missing_count > 0:
            if missing_pct > 50:
                recommendations.append(
                    f"⚠️ High missing data ({missing_pct:.1f}%) - Consider if this dataset is suitable or collect more data"
                )
            elif missing_pct > 10:
                recommendations.append(
                    f"Handle {missing_count} missing values using appropriate imputation method (mean/median/mode)"
                )
            else:
                recommendations.append(
                    f"Remove rows with missing values ({missing_count} cells affected)"
                )

        duplicate_count = data_quality_info.get("duplicate_count", 0)
        if duplicate_count > 0:
            recommendations.append(
                f"Remove {duplicate_count} duplicate rows to ensure data quality"
            )

        type_issues = data_quality_info.get("type_issues", 0)
        if type_issues > 0:
            recommendations.append(
                f"Fix data type inconsistencies in {type_issues} columns for accurate analysis"
            )

        columns_with_missing = data_quality_info.get("columns_with_missing", [])
        if len(columns_with_missing) > 5:
            recommendations.append(
                f"Focus on cleaning key columns: {', '.join(columns_with_missing[:5])}"
            )

        if not recommendations:
            recommendations.append(
                "✅ Data quality is excellent! No major cleaning needed."
            )
