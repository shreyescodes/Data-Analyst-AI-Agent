...
class DataCleaning:
        ...
        for col in df_cleaned.columns:
            if df_cleaned[col].dtype == "object":
                ...
                for pattern in date_patterns:
                    if (
                        sample_values.astype(str).str.match(pattern).sum()
    ...
    def get_cleaning_suggestions(self, df: pd.DataFrame) -> List[str]:
        ...
        if quality_info["duplicate_count"] > 0:
            suggestions.append(
                f"Remove {quality_info['duplicate_count']} duplicate rows"
            )

        if quality_info["type_issues"] > 0:
            suggestions.append(
                f"Fix data types for {quality_info['type_issues']} columns"
            )
