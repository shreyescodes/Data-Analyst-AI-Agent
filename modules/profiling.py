                    ...
                    col_stats.update(
                        {
                            ...
                            "min_length": (
                                data[col].astype(str).str.len().min()
                                if len(data) > 0
                                else 0
                            ),
                            "avg_length": (
                                data[col].astype(str).str.len().mean()
                                if len(data) > 0
                                else 0
                            ),
                        }
                ...
                report.update(
                    {
                        "statistics": {
                            ...
                            "mode": (
                                clean_data.mode().iloc[0]
                                if len(clean_data.mode()) > 0
                                else None
                            ),
                            "std": clean_data.std(),
                            "var": clean_data.var(),
                ...
                report.update(
                    {
                        "datetime_analysis": {
                            ...
                            "most_common_year": (
                                clean_data.dt.year.mode().iloc[0]
                                if len(clean_data) > 0
                                else None
                            ),
                            "most_common_month": (
                                clean_data.dt.month.mode().iloc[0]
                                if len(clean_data) > 0
                                else None
                            ),
                            "most_common_day": (
                                clean_data.dt.day.mode().iloc[0]
                                if len(clean_data) > 0
                                else None
                            ),
                        }
    ...
    def _analyze_text_patterns(self, data: pd.Series) -> dict:
        ...
        try:
            ...
            patterns = {
                ...
                "contains_special_chars": text_data.str.contains(
                    r"[^a-zA-Z0-9\s]"
                ).sum(),
                ...
                "phone_like": text_data.str.contains(
                    r"\d{3}[-.]?\d{3}[-.]?\d{4}"
                ).sum(),
            }
        ...
        try:
            ...
            comparison_data = {
                ...
                "comparison": {
                    ...
                    "unique_to_dataset_1": list(
                        set(data1.columns) - set(data2.columns)
                    ),
                    "unique_to_dataset_2": list(
                        set(data2.columns) - set(data1.columns)
                    ),
                },
            }
