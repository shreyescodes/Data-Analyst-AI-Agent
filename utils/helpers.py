...
def clean_column_names(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Clean column names
    df_copy.columns = [
        re.sub(r"[^a-zA-Z0-9_]", "_", str(col)).lower() for col in df_copy.columns
    ]

    # Ensure columns don't start with numbers
    df_copy.columns = [
        f"col_{col}" if col[0].isdigit() else col for col in df_copy.columns
    ]
    ...
    for col in df_copy.columns:
        ...
        if df_copy[col].dtype == "object":
            ...
            try:
                ...
                cleaned = (
                    df_copy[col].astype(str).str.replace(r"[\$,\%]", "", regex=True)
                )
    ...
    if len(categorical_cols) > 0:
        ...
        for col in categorical_cols:
            categorical_info[col] = {
                ...
                "most_frequent": (
                    df[col].mode().iloc[0] if len(df[col].mode()) > 0 else None
                ),
...
def generate_sample_queries(columns: List[str], dtypes: Dict[str, Any]) -> List[str]:
    ...
    if len(numeric_cols) >= 2:
        queries.append(
            f"SELECT {numeric_cols[0]}, {numeric_cols[1]} FROM data_table WHERE {numeric_cols[0]} > (SELECT AVG({numeric_cols[0]}) FROM data_table)"
        )
...
def export_data_to_formats(
    df: pd.DataFrame, formats: List[str] = None
) -> Dict[str, bytes]:
    ...
    if "json" in formats:
        exports["json"] = df.to_json(orient="records", date_format="iso").encode(
            "utf-8"
        )
            ...
            col_info.update(
                {
                    ...
                    "Min Length": (
                        df[col].astype(str).str.len().min() if len(df) > 0 else 0
                    ),
                    "Sample Values": str(df[col].dropna().head(3).tolist()),
                }
            )
...
def log_activity(
    activity: str, details: Dict[str, Any] = None, log_file: str = "activity.log"
):
...
def generate_data_quality_score(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    uniqueness_score = (
        ((len(df) - duplicate_rows) / len(df)) * 100 if len(df) > 0 else 100
    )
    ...
    return {
        ...
        "recommendations": get_quality_recommendations(
            completeness_score, uniqueness_score, type_consistency_score
        ),
    }
...
def get_quality_recommendations(
    completeness: float, uniqueness: float, consistency: float
) -> List[str]:
